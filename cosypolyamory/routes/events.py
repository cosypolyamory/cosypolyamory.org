...
# - Attendance yes: ordered list of users (first come first serve)
# - Attendance no: ordered list of users.
# - Attendance maybe: ordered list of users.
# - update: boolean.
# 
...
#  - If the additions would make the event go over capacity, reject all, abort the DB transaction
#    set an informative flash message.
#  - If all is within limits, commit the transaction.
#
# When update is true:
#   Carry out the steps above, but before committing the transaction, also:
...
def manage_attendance(event_id):
    ...
    - update: boolean - if true, also ensure hosts have RSVPs and promote waitlisted users
    ...
    # Check permissions - only admin, organizers, or event host can manage attendance
    ...
    update_mode = data.get('update', False)
    
    # Validate that all IDs are integers
    try:
        attendance_yes = [int(uid) for uid in attendance_yes]
        attendance_no = [int(uid) for uid in attendance_no]
        attendance_maybe = [int(uid) for uid in attendance_maybe]
    ...
    try:
        with database.atomic():
            ...
            # Step 1: Apply attendance_no updates first
            for user_id in attendance_no:
                try:
                    ...
                    if not created and rsvp.status != 'no':
                        old_status = rsvp.status
                        rsvp.status = 'no'
                        rsvp.updated_at = datetime.now()
                        rsvp.save()
                        updated_rsvps.append({'user': user, 'old_status': old_status, 'new_status': 'no'})
                except User.DoesNotExist:
                    database.rollback()
                    return jsonify({'success': False, 'error': f'User {user_id} not found'}), 400
            
            # Step 2: Apply attendance_yes updates
            for user_id in attendance_yes:
                try:
                    user = User.get_by_id(user_id)
                    rsvp, created = RSVP.get_or_create(
                        event=event,
                    ...
                    if not created and rsvp.status != 'yes':
                        old_status = rsvp.status
                        rsvp.status = 'yes'
                        rsvp.updated_at = datetime.now()
                        rsvp.save()
                        updated_rsvps.append({'user': user, 'old_status': old_status, 'new_status': 'yes'})
                except User.DoesNotExist:
                    database.rollback()
                    return jsonify({'success': False, 'error': f'User {user_id} not found'}), 400
            
            # Step 3: Apply attendance_maybe updates
            for user_id in attendance_maybe:
                try:
                    user = User.get_by_id(user_id)
                    rsvp, created = RSVP.get_or_create(
                        event=event,
                    ...
                    if not created and rsvp.status != 'maybe':
                        old_status = rsvp.status
                        rsvp.status = 'maybe'
                        rsvp.updated_at = datetime.now()
                        rsvp.save()
                        updated_rsvps.append({'user': user, 'old_status': old_status, 'new_status': 'maybe'})
                except User.DoesNotExist:
                    database.rollback()
                    return jsonify({'success': False, 'error': f'User {user_id} not found'}), 400
            ...
            # Step 5: If update_mode is true, ensure hosts have RSVPs and promote waitlist
            if update_mode:
        ...
        # Now send notifications for promoted users (after transaction is complete)
